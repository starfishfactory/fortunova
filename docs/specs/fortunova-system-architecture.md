...
| 단계 | 마일스톤 | 주요 산출물 | 세부 항목 |
|------|---------|-----------|----------|
| Phase 1 | 프로젝트 셋업 + 사주 엔진 핵심 | 사주팔자 계산 함수 + 50+ 테스트 통과 | **1-1** 프로젝트 초기화: npm, TypeScript, Vitest, ESLint, Hono Hello World, Docker+SQLite+Drizzle, CI |
| | | | **1-2** 사주 엔진 기초 데이터: 천간(10)/지지(12)/오행/음양 매핑, 60갑자 순환 테이블, 상생/상극 관계 (TDD) |
| | | | **1-3** 달력 변환: KASI 기반 양음력 변환(1900-2100), 24절기 데이터+절기 기반 월 판별, 진태양시 보정 (TDD) |
| | | | **1-4** 사주팔자 계산: 년주(입춘 기준)/월주(절기 기준)/일주(만세력)/시주, `calculateFourPillars()`, 50+ 케이스 교차 검증 (TDD) |
| Phase 2 | AI 풀이 연동 + 기본 UI | 운세 조회 API + 웹 UI | **2-1** 사주 분석 고도화: 십신, 오행 균형, 일간 강약 판별 |
| | | | **2-2** Claude CLI 연동: 프롬프트 빌더, 응답 파서, 캐시 저장, 에러 핸들링 |
| | | | **2-3** 기본 UI: Tailwind+HTMX 레이아웃, 생년월일시 입력 폼, 운세 결과 페이지, 모바일 반응형 |
| Phase 3 | 인증 + 무료 횟수 제한 | JWT 인증 + 일일 3회 제한 | 회원가입/로그인, JWT 미들웨어, 일일 사용량 추적(userId/fingerprint), 초과 시 구독 유도 UI, PWA(manifest+SW) |
| Phase 4 | 구독/결제 | 유료 구독 결제 흐름 | 토스페이먼츠 or 카카오페이 연동, 구독 플랜(월간/연간), 결제 API+웹훅, 마이페이지 |
| Phase 5+ | 확장 | 추가 운세 시스템 + 고급 기능 | 12운성/신살 상세, 대운/세운 차트, 타로 플러그인, 별자리 플러그인, 궁합, 푸시 알림, 소셜 로그인, 공유 기능 |
...
- **설명**: Node.js의 사실상 표준 웹 프레임워크
- **장점**: 가장 큰 생태계, 풍부한 미들웨어, 학습 자료 다수
- **단점**: 번들 크기 ~200KB (Hono의 14배), 레거시 API 설계, TypeScript 지원 추가 설정 필요, Web Standards 미준수

#### 대안 B: Fastify

- **설명**: 성능 중심 Node.js 프레임워크
- **장점**: Express 대비 2-3배 빠른 벤치마크, JSON Schema 기반 검증, TypeScript 지원
- **단점**: 번들 크기 ~350KB, 플러그인 시스템 학습 곡선, HTMX와의 SSR 통합 사례 부족

#### 대안 C: Hono v4 (선택)

- **설명**: 14KB 초경량 Web Standards 프레임워크
- **장점**: 14KB 번들, Web Standards API 기반, JSX 기반 SSR 내장, 미들웨어 풍부, TypeScript 네이티브
- **단점**: Express/Fastify 대비 생태계 작음, 상대적으로 신규 프레임워크

**선택 근거**: 512MB Docker 환경에서 메모리 풋프린트 최소화가 최우선이며, HTMX + JSX SSR의 네이티브 지원으로 추가 템플릿 엔진이 불필요하다. 14KB 번들 크기는 Express(200KB)/Fastify(350KB) 대비 메모리 절약에 유리하다.

### 9.2 데이터베이스

#### 대안 A: PostgreSQL (Docker)

- **설명**: 별도 Docker 컨테이너로 PostgreSQL 운영
- **장점**: 동시성 처리 우수(MVCC), 확장성, 풍부한 데이터 타입(JSONB), 성숙한 생태계
- **단점**: 별도 컨테이너 ~100-200MB 추가 메모리, 2GB RAM NAS에서 Node.js + PostgreSQL 동시 운영 시 OOM 위험, 관리 복잡도 증가

#### 대안 B: SQLite + better-sqlite3 (선택)

- **설명**: 파일 기반 임베디드 데이터베이스
- **장점**: 별도 프로세스 없음(~10-20MB RSS), 파일 기반 백업 용이, 동기 API로 코드 단순, NAS 볼륨 마운트와 자연스러운 통합
- **단점**: 단일 프로세스 쓰기 잠금, 동시 접속 다수 시 병목, 수평 확장 불가

**선택 근거**: NAS 2GB RAM에서 PostgreSQL 별도 컨테이너(~100-200MB)를 추가하면 메모리 한계에 도달한다. MVP 단계에서 예상 동시 접속 수(10명 이내)는 SQLite WAL 모드로 충분히 처리 가능하며, 사용자 증가 시 PostgreSQL 마이그레이션을 검토한다.

### 9.3 LLM 연동 방식

#### 대안 A: Claude API SDK (@anthropic-ai/sdk)

- **설명**: Anthropic 공식 TypeScript SDK를 직접 사용
- **장점**: 타입 안전성, 스트리밍 지원, 구조화된 에러 핸들링, 직접적인 API 제어
- **단점**: SDK 의존성 추가(번들 크기 증가), API 키 관리 필요, SDK 버전 업데이트 추적 필요

#### 대안 B: GPT API (OpenAI)

- **설명**: OpenAI GPT-4 API 사용
- **장점**: 가장 큰 생태계, 다양한 모델 선택지
- **단점**: 한국어 품질이 Claude 대비 낮음(특히 사주/명리 도메인), 비용이 상대적으로 높음

#### 대안 C: Claude CLI headless mode (선택)

- **설명**: Docker 내 Claude CLI를 설치하고 `--headless` 플래그로 프로세스 호출
- **장점**: SDK 의존성 제거, 프로세스 호출로 격리, CLI 자체의 대화 관리 기능 활용 가능, API 키를 CLI 설정에 위임
- **단점**: 프로세스 스폰 오버헤드(~100-200ms), stdout 파싱 필요, CLI 버전 의존성, Docker 이미지 크기 증가

**선택 근거**: Claude의 한국어 품질(특히 사주/명리 도메인)이 GPT 대비 우수하며, CLI headless 모드는 SDK 의존성을 제거하고 프로세스 격리로 메모리 관리가 용이하다. 프로세스 스폰 오버헤드는 캐시 히트율 80%+ 목표로 상쇄한다.

### 9.4 런타임 환경

#### 대안 A: Bun

- **설명**: 올인원 JavaScript 런타임
- **장점**: 빠른 성능, 내장 번들러/테스트러너
- **단점**: Docker 이미지 ~240MB (Node.js Alpine ~80MB의 3배), 메모리 누수 보고 존재, NAS 환경에서 안정성 미검증

#### 대안 B: Node.js 20 LTS Alpine (선택)

- **설명**: LTS 지원 Node.js + 경량 Alpine Linux
- **장점**: Docker 이미지 ~80MB, 안정성 검증됨, 생태계 최대, better-sqlite3 호환 확인
- **단점**: Bun 대비 순수 성능 낮음

**선택 근거**: NAS 2GB RAM 환경에서 Docker 이미지 크기와 메모리 안정성이 최우선이다. Node.js 20 LTS Alpine은 ~80MB 이미지 + ~40-60MB RSS로 512MB 제한 내에서 가장 안전한 선택이다.
...
| 용어 | 정의 |
|------|------|
| 천간(天干) | 갑·을·병·정·무·기·경·신·임·계의 10개 요소. 하늘의 기운을 나타냄 |
| 지지(地支) | 자·축·인·묘·진·사·오·미·신·유·술·해의 12개 요소. 땅의 기운을 나타냄 |
| 간지(干支) | 천간과 지지의 조합. 60갑자를 구성하는 기본 단위 |
| 오행(五行) | 목·화·토·금·수의 5가지 요소. 만물의 구성과 변화를 설명하는 원리 |
| 사주팔자(四柱八字) | 년·월·일·시의 4개 기둥(柱)에 각 천간+지지 2글자씩, 총 8글자(八字) |
| 십신(十神) | 일간(日干)을 기준으로 다른 7개 천간/지지와의 관계를 나타내는 10가지 분류 |
| 일간(日干) | 일주의 천간. 사주에서 "나 자신"을 나타내는 핵심 요소 |
| 용신(用神) | 사주의 균형을 맞추기 위해 필요한 오행. 운세 풀이의 핵심 기준 |
| 대운(大運) | 10년 주기로 변화하는 운의 흐름 |
| 세운(歲運) | 매년 바뀌는 운의 흐름 |
| 입춘(立春) | 양력 2월 4일경. 사주에서 년주가 바뀌는 기준점 |
| 절기(節氣) | 24절기. 사주에서 월주가 바뀌는 기준점 (절기 기준, 중기 제외) |
| 야자시/조자시 | 23:00-01:00 시간대의 시주 판별 방식. 야자시(밤 자시)는 전일 기준, 조자시(아침 자시)는 당일 기준 |
| 만세력(萬歲曆) | 1만년의 달력. 특정 날짜의 간지를 조회할 수 있는 역법 데이터 |
| PWA | Progressive Web App. 웹 기술로 네이티브 앱과 유사한 경험을 제공하는 웹 애플리케이션 |
| WAL 모드 | Write-Ahead Logging. SQLite의 동시성 개선 모드. 읽기와 쓰기를 동시에 처리 가능 |
| RSS | Resident Set Size. 프로세스가 실제로 사용 중인 물리 메모리 크기 |

### 10.2 참고 자료

- [fortunova-plan.md](fortunova-plan.md) - 초기 기획 문서
- [Hono 공식 문서](https://hono.dev) - 서버 프레임워크
- [HTMX 공식 문서](https://htmx.org) - 프론트엔드 인터랙션
- [Drizzle ORM 공식 문서](https://orm.drizzle.team) - ORM
- [better-sqlite3 GitHub](https://github.com/WiseLibs/better-sqlite3) - SQLite 바인딩
- [Claude CLI 문서](https://docs.anthropic.com/en/docs/claude-code) - LLM 연동
- 만세력닷컴 - 사주 계산 교차 검증 소스
- 더큼만세력 - 사주 계산 교차 검증 소스
- 척척만세력 - 사주 계산 교차 검증 소스
...
| 항목 | 예상 RSS | 비고 |
|------|---------|------|
| Node.js 기본 | ~40MB | V8 엔진 기본 힙 |
| Hono + 미들웨어 | ~5MB | 14KB 번들 + 런타임 객체 |
| SQLite (better-sqlite3) | ~10-20MB | WAL 모드, 캐시 포함 |
| 사주 데이터 (in-memory) | ~5-10MB | 1900-2100 음력/절기 데이터 |
| 요청 처리 버퍼 | ~20-50MB | 동시 요청 처리 시 피크 |
| **합계** | **~80-125MB** | 512MB 제한의 16-24% 사용 |
| Claude CLI 프로세스 (일시적) | ~50-100MB | 호출 시에만 존재, 종료 후 해제 |
